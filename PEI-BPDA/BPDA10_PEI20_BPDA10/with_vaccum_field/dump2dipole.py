...
def dump2dipole(fName,groups,flagDebug=False, hardCoded=False):
    ...
    with open(fName,'r') as f:
        ...
        # compute the volume
        xtem=np.array(next(f).strip().split(),dtype=float)
        ytem=np.array(next(f).strip().split(),dtype=float)
        ztem=np.array(next(f).strip().split(),dtype=float)
        if len(xtem) == 3:
            xlo,xhi, xy=xtem
            ylo,yhi, yz=ytem
            zlo,zhi, xz=ztem
        ...
        for i_line in range(N_lines):
            ...
            qxyz[atom_id-1,:]=np.array([words[t] for t in qxyz_index],dtype=float)
